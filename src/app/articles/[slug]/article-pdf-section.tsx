...
function PdfItem({ pdf, idx, articleId, articleTitle }: PdfItemProps) {
  ...
  if (pdf.isRestricted && !pdf.accessGranted) {
    ...
    return (
      <div className="mb-8 rounded-lg border shadow-sm bg-white overflow-hidden">
        <div className="w-full aspect-[16/9] min-h-[400px] bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
  ...
  return (
    <div className="mb-8 rounded-lg border shadow-sm bg-white overflow-hidden animate-fade-in">
...
export default function ArticlePdfSection({ pdfs, articleId, articleTitle }: ArticlePdfSectionProps) {
  ...
  return (
    <div className="my-8">
      <h2 className="text-xl font-semibold mb-3 text-[#4c6b54] font-serif border-b pb-2">Tài liệu đính kèm (PDF)</h2>
      {pdfs.map((pdf, idx) => (
        <PdfItem 
          key={pdf.url + idx} 
          pdf={pdf} 
