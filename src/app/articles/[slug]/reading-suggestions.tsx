    ...
    <section>
      <Card className="border-2 border-[#e6dfd3] bg-[#fcf9f2]/70 rounded-md overflow-hidden shadow-sm font-sans">
        <CardHeader className="pb-3 border-b border-[#e6dfd3]">
          ...
          <p className="text-muted-foreground text-xs font-serif mt-1">
        ...
        <CardContent className="pt-5">
          {!isLoading && !hasFetched && suggestions.length === 0 && (
             <div className="flex flex-col items-center justify-center text-center p-6 border-dashed border-2 border-[#e6dfd3] rounded-lg bg-white/50">
                <p className="text-muted-foreground text-sm mb-4 font-serif">Nhấn nút để nhận đề xuất các bài đọc liên quan.</p>
                <Button onClick={handleGetSuggestions} disabled={isLoading} className="bg-[#4c6b54] hover:bg-[#3d5643] text-white">
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center p-8">
              ...
              <span className="text-muted-foreground text-sm font-serif">Đang tìm kiếm gợi ý đọc thêm...</span>
            </div>
          )}

          {!isLoading && error && (
            <div className="text-center p-6 bg-white/50 border border-[#e6dfd3] rounded-lg">
              <p className="text-destructive text-sm font-serif mb-3">{error}</p>
              <Button onClick={handleGetSuggestions} variant="outline" className="border-[#4c6b54] text-[#4c6b54] hover:bg-[#fcf9f2]">Thử lại</Button>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {suggestions.map((article) => (
                  <Link key={article.slug} href={`/articles/${article.slug}`}>
                    <Card className="h-full border border-[#e6dfd3] bg-white hover:border-[#4c6b54] hover:shadow-md transition-all group duration-300">
                      ...
                      <CardContent className="p-4 pt-0 text-xs">
                        <p className="text-muted-foreground line-clamp-3 mb-2 font-serif">
                          {article.excerpt}
                        </p>
                        <div className="flex justify-between items-center text-slate-500 font-serif mt-3 pt-2 border-t border-slate-100">
