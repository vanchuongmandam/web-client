...
import { Loader2, Upload, X, Image as ImageIcon, FileText } from 'lucide-react'; // Added FileText icon
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (articleData) {
                    ...
                    // Ensure content is passed as JSON object
                    form.reset({ ...articleData, category: articleData.category._id, content: articleData.content || { "type": "doc", "content": [{ "type": "paragraph", "content": [{ "type": "text", "text": "" }] }] } });
                } else {
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)}>
                    <header className="flex items-center justify-between mb-8">
                        <div><h1 className="text-4xl font-sans font-bold text-primary">Chỉnh sửa bài viết</h1><p className="text-muted-foreground mt-2">Cập nhật: <span className="font-semibold text-foreground">{form.getValues('title')}</span></p></div>
