...
async function getArticles(): Promise<Article[]> {
  try {
    ...
    // Sử dụng cache 'no-store' để đảm bảo dữ liệu luôn mới khi phát triển
...
export default async function Home() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* --- Trending Section (Toàn chiều rộng) --- */}
      {trendingArticles.length > 0 && (
        <section className="mb-12">
          <h2 className="font-headline text-3xl font-bold mb-6">Xu hướng</h2>
          <Carousel opts={{ loop: trendingArticles.length > 2 }} className="w-full">
      ...
      <div className="space-y-12">
        ...
        {/* Newspaper Section (Chuyển xuống đây) */}
      ...
      <Card className="overflow-hidden h-full transition-all duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl">
        <CardContent className="p-0">
          <div className="relative aspect-[3/4] w-full bg-muted">
            {imageUrl && (<Image src={imageUrl} alt={article.title} fill className="object-cover transition-transform duration-300 group-hover:scale-105"/>)}
          </div>
          <div className="p-3">
